...
- 誤差！の説明などをして、統計が神がもたらす結果みたいな安易な考えを持たせない。
...
ggplot(score_data, aes(x = "英語", y = English, fill = "英語")) +
  ...
  labs(title = "英語と国語の得点分布", x = "科目", y = "得点") +
    stat_summary(fun.y = median, geom = "text", aes(label = paste("中央値")),
               vjust = -0.2, hjust = -3, color = "black", size = 4) +
  ...
  annotate("text", x = 1.1, y = quantile(score_data$English, prob = 0.75)*1.42, label = "外れ値", color = "red", size = 4) + stat_boxplot(geom='errorbar', width = 0.05) +
  annotate("text", x = 1.15, y = min(score_data$English), label = "最小値", color = "blue", size = 4) +
  annotate("text", x = 0.85, y = max(score_data$English)*0.97, label = "最大値", color = "blue", size = 4) 
```
...
```{r}
ggplot(vote_counts, aes(x = Pokemon, y = Frequency)) +
  geom_point(aes(size = Frequency), shape = 21, fill = "skyblue", color = "black", alpha = 0.7) +
  scale_size(range = c(3, 10)) +  # バルーンの大きさ調整
  labs(title = "ポケモン人気投票 バルーンプロット", x = "ポケモン", y = "投票数") +
  theme_minimal()
